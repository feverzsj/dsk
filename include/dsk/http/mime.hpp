...
struct mime_info
{
    ...
    std::string_view exts; // Comma-separated list of possible extensions for the type. The first extension is considered preferred.
};
...
constexpr std::string_view ext_to_mime(std::string_view ext, //vvv: for textual file, use 'text/plain'
                                       std::string_view const& def = "application/octet-stream") noexcept
{
    if(ext.size() > 65536)
    {
        return def; // Avoids crash when unable to handle a long file path. See crbug.com/48733.
    }

    if(ext.find('\0') != npos)
    {
        return def; // Reject a string which contains null character.
    }
    ...
    {
        std::string_view const& exts = info.exts;
...
constexpr std::string_view mime_to_ext(std::string_view const& mime, std::string_view const& def = {}) noexcept
{

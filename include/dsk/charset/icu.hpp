...
class icu_err_category : public error_category
{
public:
...
inline constexpr icu_err_category g_icu_err_cat;

} // namespace dsk


static_assert(U_ZERO_ERROR == 0);

DSK_SPECIALIZE_ERROR_CODE_ENUM(UErrorCode)

constexpr dsk::error_code make_error_code(UErrorCode e) noexcept
{
    DSK_ASSERT(e > 0);
    return {static_cast<int>(e), dsk::g_icu_err_cat};
}
...
{
    return ucnv_compareNames(cstr_data(as_cstr(n1)), cstr_data(as_cstr(n2)));
}
...
template<class T>
using icu_expected = expected<T, UErrorCode>;
...
class icu_charset
{
    copyable_handle<
        ...
        [](UConverter* h)
        {
            ...
            UConverter* t = ucnv_safeClone(h, nullptr, nullptr, &e);
    ...
    explicit icu_charset(UConverter* h) noexcept : _h(h) {}
...
public:
    ...
    static icu_expected<icu_charset> open(_char_str_ auto const& name)
    {
        UErrorCode e = U_ZERO_ERROR;
        
        if(UConverter* h = ucnv_open(cstr_data(as_cstr(name)), &e))
        {
    ...
    icu_expected<char const*> name() const
    {
        UErrorCode e = U_ZERO_ERROR;

        if(char const* n = ucnv_getName(handle(), &e))
        {
            return n;
        }
    ...
    UErrorCode from_u16(_u16_str_ auto const& s, _resizable_byte_buf_ auto& d)
    {
        if constexpr(! SkipMaxResize)
            rescale_buf(d, max_from_u16_result_size(s));
    ...
    template<bool SkipMaxResize = false>
    UErrorCode to_u16(_byte_str_ auto const& s, _resizable_16bits_buf_ auto& d)
    {
        if constexpr(! SkipMaxResize)
            rescale_buf(d, max_to_u16_result_size(s));
    ...
    icu_expected<B> from_u16(_u16_str_ auto const& s)
    {
        B d;
        DSK_E_TRY_ONLY(from_u16(s, d));
        return d;
    }

    template<_resizable_16bits_buf_ B = u16string>
    icu_expected<B> to_u16(_byte_str_ auto const& s)
    {
        B d;
        DSK_E_TRY_ONLY(to_u16(s, d));
        return d;
    }
};
...
template<bool SkipMaxResize = false>
UErrorCode cvt_charset(icu_charset& srcCodec, icu_charset& dstCodec,
                       _byte_str_ auto const& s, _resizable_byte_buf_ auto& d)
{
    DSK_E_TRY_FWD(u16s, to_u16(s));
    return dstCodec.template from_u16<SkipMaxResize>(u16s, d);
}
...
public:
    ...
    static icu_expected<icu_charset_cvt> open(_char_str_ auto const& srcName, _char_str_ auto const& dstName)
    {
        icu_charset_cvt cvt;

        DSK_E_TRY(cvt.src, icu_charset::open(srcName));
        DSK_E_TRY(cvt.dst, icu_charset::open(dstName));

        return cvt;
    }

    static UErrorCode to_dst(_char_str_ auto const& srcCs, _byte_str_ auto const& s,
                             _char_str_ auto const& dstCs, _resizable_byte_buf_ auto& d)
    {
        DSK_E_TRY_FWD(cvt, open(srcCs, dstCs));
        return cvt.forward(s, d);
    }

    template<_resizable_byte_buf_ B = string>
    static icu_expected<B> to_dst(_char_str_ auto const& srcCs, _byte_str_ auto const& s,
                                  _char_str_ auto const& dstCs)
    {
        B d;
        DSK_E_TRY_ONLY(to_dst(srcCs, s, dstCs, d));
        return d;
    }

    // src => dst
    template<bool SkipMaxResize = false>
    UErrorCode forward(_byte_str_ auto const& s, _resizable_byte_buf_ auto& d)
    {
    ...
    // dst => src
    template<bool SkipMaxResize = false>
    UErrorCode inverse(_byte_str_ auto const& s, _resizable_byte_buf_ auto& d)
    {
